...
template <typename T> class DoublyLinkedList {
private:
  // Node now contains a pointer to the previous node
  struct Node {
    T m_data;
    Node *m_next;
    Node *m_prev; // The key addition for a doubly-linked list
    Node(T data) : m_data(data), m_next(nullptr), m_prev(nullptr) {}
  };

  Node *m_head;
  Node *m_tail; // A tail pointer for O(1) back operations
  size_t m_size;
  ...
  void updatePrevPointersAndTail(); // Helper to fix prev pointers after sort
...
public:
  ...
  // --- Modifiers (push_back() and pop_back() are now O(1)) ---
...
template <typename T> const T &DoublyLinkedList<T>::at(size_t pos) const {
    if (pos >= m_size) throw std::out_of_range("Index out of range");
    // Re-use the non-const version's logic to avoid code duplication
...
template <typename T> void DoublyLinkedList<T>::push_front(T data) {
  ...
  m_head = newNode;
  if (m_tail == nullptr) { // If list was empty
    m_tail = m_head;
  }
  m_size++;
}
...
template <typename T> void DoublyLinkedList<T>::reverse() {
  ...
  while (current != nullptr) {
    ...
    current = current->m_prev; // Move to the original next
  }
...
template <typename T> void DoublyLinkedList<T>::sort() {
  ...
  // After sorting, the prev pointers are broken. Fix them.
...
void DoublyLinkedList<T>::updatePrevPointersAndTail() {
    ...
    m_tail = current; // The last node is the new tail
}
...
int main() {
  ...
  // Test 1: push_back and push_front
